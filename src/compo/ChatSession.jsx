...
const ChatSession = ({ onChatSelect, activeSession }) => {
  ...
  return (
    <div className="chatSession w-full h-full space-y-2 border-r-[1px] border-white overflow-auto">
      <div className="heading w-full sm:h-[3vw] h-[5vh] flex pl-7 items-center justify-start font-semibold bg-[#00A0AE] text-lg">
      ...
      <div className="chatsession border-t-[1px] w-full border-zinc-300">
        {filteredSessions.map((session) => (
          <div
            key={session.id}
            onClick={() => onChatSelect(`chat${session.id}`, session.id)} // Pass chatId and userId
            className={`w-full ${activeSession === session.id ? 'border-b-[1px] border-t-[1px] duration-300 bg-zinc-200' : 'bg-white'} cursor-pointer`} // Change background if active
          >
            <SingleSession
              name={session.name}
